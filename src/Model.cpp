...
void Model::SetAltitude(State& state, double AltitudeMeanSeaLevel_ft) {
	// Set the altitude
...
void Model::SetMach(State& state, double MachNumber) {
	// Update true airspeed
	double AltitudeMeanSeaLevel_m = state.AltitudeMeanSeaLevel_ft * FT_TO_M;
	double AirTemperature_k = atmosphere.Temperature(AltitudeMeanSeaLevel_m);
	double SpeedOfSound_fps = atmosphere.SpeedofSound(AirTemperature_k) / FT_TO_M;
	state.TrueAirspeed_fps = SpeedOfSound_fps * MachNumber;
	// Propegate states
	double alpha_rad = state.Alpha_deg * DEG_TO_RAD;
	double beta_rad = state.Beta_deg * DEG_TO_RAD;
...
void Model::SetAlpha(State& state, double alpha_deg) {
...
void Model::Atmosphere(State& state) {
	// Update Atmospherics - Careful with Units
	double AltitudeMeanSeaLevel_m = state.AltitudeMeanSeaLevel_ft * FT_TO_M;
	// Temperature
	double AirTemperature_k = atmosphere.Temperature(AltitudeMeanSeaLevel_m); 
	state.AirTemperature_r = AirTemperature_k * K_TO_R; // Kelvin to Rankine 
	// Pressure
	double StaticPressure_pa = atmosphere.Pressure(AltitudeMeanSeaLevel_m);
	state.StaticPressure_psf = StaticPressure_pa * PA_TO_PSF;
	// Density
	double AirDensity_kgm3 = atmosphere.Density(AirTemperature_k, StaticPressure_pa);
	state.AirDensity_slugft3 = AirDensity_kgm3 / SLUGFT3_TO_KGM3;
	// Speed of Sound
	double SpeedOfSound_ms = atmosphere.SpeedofSound(AirTemperature_k);
	state.SpeedOfSound_kt = (SpeedOfSound_ms / FT_TO_M) / KT_TO_FTS;
	state.SpeedOfSound_fps = state.SpeedOfSound_kt * KT_TO_FTS;
	// True Airspeed
	state.TrueAirspeed_fps = std::sqrt(std::pow(state.U_fps, 2) + std::pow(state.V_fps, 2) + std::pow(state.W_fps, 2));
	state.TrueAirspeed_kt = state.TrueAirspeed_fps / KT_TO_FTS;
	// Mach Number
	...
	// Equivilant Airspeed
	state.EquivilantAirspeed_fps = SpeedOfSound_SeaLevel_fps * state.MachNumber * std::sqrt(state.StaticPressure_psf / StaticPressure_SeaLevel_psf);
	state.EquivilantAirspeed_kt = state.EquivilantAirspeed_fps / KT_TO_FTS;
};


void Model::AeroAngles(State& state) {
	// Aero angles
	state.Alpha_deg = atan2(state.W_fps, state.U_fps) * RAD_TO_DEG;
	state.Beta_deg = asin(state.V_fps / state.TrueAirspeed_fps) * RAD_TO_DEG;
...
void Model::LonAero(State& state) {
	// Angles in rads/sec
	double alpha_rad = state.Alpha_deg * DEG_TO_RAD;
	double q_rad = state.Q_dps * DEG_TO_RAD;
	double alpha_dot_rps = state.Alpha_dot_dps * DEG_TO_RAD;
	double beta_dot_rps = state.Beta_dot_dps * DEG_TO_RAD;
	double sin_alpha = sin(alpha_rad);
	double cos_alpha = cos(alpha_rad);
	// Stab position
	double stab_rad = state.StabCommand_deg * DEG_TO_RAD;
	// CL
	double CL_Total = state.CL0;
	CL_Total += state.CL_alpha * alpha_rad;
	CL_Total += state.CL_alpha_dot * alpha_dot_rps * ReferenceChord_ft / (2.0 * state.TrueAirspeed_fps);
	CL_Total += state.CL_q * q_rad * ReferenceChord_ft / (2 * state.TrueAirspeed_fps);
	...
	CM_Total += state.CM_alpha_dot * alpha_dot_rps * ReferenceChord_ft / (2.0 * state.TrueAirspeed_fps);
	CM_Total += state.CM_q * q_rad * ReferenceChord_ft / (2 * state.TrueAirspeed_fps);
	...
	// Sum of Forces
	state.Lon_FX_lbs = (CL_Total * sin_alpha - CD_Total * cos_alpha) * state.DynamicPressure_psf * ReferenceWingArea_ft2;
	state.Lon_FY_lbs = 0;
	state.Lon_FZ_lbs = (-CL_Total * cos_alpha - CD_Total * sin_alpha) * state.DynamicPressure_psf * ReferenceWingArea_ft2;;
	// Sum of Moments
	state.Lon_MX_ftlbs = 0;
	state.Lon_MY_ftlbs = CM_Total * state.DynamicPressure_psf * ReferenceWingArea_ft2 * ReferenceChord_ft;
	state.Lon_MZ_ftlbs = 0;
};


void Model::LatDirAero(State& state) {
	// Convert angles and rates to radians
	double beta_rad = state.Beta_deg * DEG_TO_RAD;
	double alpha_rad = state.Alpha_deg * DEG_TO_RAD;
	double p_rad = state.P_dps * DEG_TO_RAD;
	double r_rad = state.R_dps * DEG_TO_RAD;
	double aileron_rad = state.AileronCommand_deg * DEG_TO_RAD;
	double rudder_rad = state.RudderCommand_deg * DEG_TO_RAD;
	// Airspeed from state
	...
	Cl_Total += state.Cl_p * p_rad * ReferenceSpan_ft / (2.0 * V_fps);
	Cl_Total += state.Cl_r * r_rad * ReferenceSpan_ft / (2.0 * V_fps);
	Cl_Total += state.Cl_delta_ail * aileron_rad;
	Cl_Total += state.Cl_delta_rud * rudder_rad;
	// Cn (Yawing moment coefficient)
	double Cn_Total = state.CN_b * beta_rad;
	Cn_Total += state.CN_p * p_rad * ReferenceSpan_ft / (2.0 * V_fps);
	Cn_Total += state.CN_r * r_rad * ReferenceSpan_ft / (2.0 * V_fps);
	...
	state.LatDir_FY_lbs = CY_Total * state.DynamicPressure_psf * ReferenceWingArea_ft2;
	state.LatDir_FZ_lbs = 0;
	// Aerodynamic Moments
	state.LatDir_MX_ftlbs = Cl_Total * state.DynamicPressure_psf * ReferenceWingArea_ft2 * ReferenceSpan_ft;
	state.LatDir_MY_ftlbs = 0;
	state.LatDir_MZ_ftlbs = Cn_Total * state.DynamicPressure_psf * ReferenceWingArea_ft2 * ReferenceSpan_ft;
};


void Model::Propulsion(State& state) {
	// Thrust
	state.Thrust_lb = state.Throttle_norm * ThrustLimit_lbs;
	// Sum of Forces
...
void Model::EquationsOfMotion(State& state) {
	// Convert degrees to radians
	double phi_rad = state.Phi_deg * DEG_TO_RAD;
	double theta_rad = state.Theta_deg * DEG_TO_RAD;
	double psi_rad = state.Psi_deg * DEG_TO_RAD;
	double p_rad = state.P_dps * DEG_TO_RAD;
	double q_rad = state.Q_dps * DEG_TO_RAD;
	double r_rad = state.R_dps * DEG_TO_RAD;
	// Trig values
	...
	// Gravity
	double mass_sl = state.AircraftWeight_lbs / AccelGravity_fts2;
	double Gravity_FX_lbs = -state.AircraftWeight_lbs * sin_theta;
	double Gravity_FY_lbs = state.AircraftWeight_lbs * cos_theta * sin_phi;
	double Gravity_FZ_lbs = state.AircraftWeight_lbs * cos_theta * cos_phi;
	// Sum of Forces
	...
	state.P_dot_dps2 = p_dot_rps2 * RAD_TO_DEG;
	state.Q_dot_dps2 = q_dot_rps2 * RAD_TO_DEG;
	state.R_dot_dps2 = r_dot_rps2 * RAD_TO_DEG;
};


void Model::Integration(State& state) {
	// Trig values
	double phi_rad = state.Phi_deg * DEG_TO_RAD;
	double theta_rad = state.Theta_deg * DEG_TO_RAD;
	double cos_phi = cos(phi_rad);
